 ...
 *
 * This is a modified version of json.h, a public domain library available at
...
static struct json_value_s *json_parse(const void *src, size_t src_size);
...
static void *json_write_minified(const struct json_value_s *value,
				 size_t *out_size);
 ...
 * array. */
static struct json_array_s *
json_value_as_array(struct json_value_s *const value);

/* Whether the value is true. */
static int json_value_is_true(const struct json_value_s *const value);

/* Whether the value is false. */
static int json_value_is_false(const struct json_value_s *const value);

/* Whether the value is null. */
static int json_value_is_null(const struct json_value_s *const value);
...
static int json_hexadecimal_value(const char *c, const unsigned long size,
				  unsigned long *result);
static int
json_hexadecimal_value(const char *c, const unsigned long size,
		       unsigned long *result)
{
	const char *p;
	int digit;
	...
	while ((offset < size) && (quote_to_use != src[offset])) {
		...
		if ('\\' == src[offset]) {
			...
			case 'u':
				...
				codepoint = 0;
				if (!json_hexadecimal_value(&src[offset + 1], 4,
							    &codepoint)) {
...
json_get_key_size(struct json_parse_state_s *state)
{
	...
	if (json_parse_flags_allow_unquoted_keys & flags_bitset) {
		...
		if ('"' == src[offset]) {
			...
			while ((offset < size) &&
			       is_valid_unquoted_key_char(src[offset])) {
				offset++;
				data_size++;
			}
			...
			if (json_parse_flags_allow_location_information &
			    flags_bitset) {
				state->dom_size +=
					sizeof(struct json_string_ex_s);
			} else {
				state->dom_size += sizeof(struct json_string_s);
			}
       ...
       * stream ends! */
		...
		if (allow_comma) {
			if (',' == src[state->offset]) {
				...
				state->offset++;
				allow_comma = 0;
			} else if (json_parse_flags_allow_no_commas &
				   flags_bitset) {
			...
			if (json_parse_flags_allow_trailing_comma &
			    flags_bitset) {
				continue;
			} else {
				if (json_skip_all_skippables(state)) {
...
json_get_array_size(struct json_parse_state_s *state)
{
	...
	while (state->offset < size) {
		...
		if (']' == src[state->offset]) {
			...
			state->dom_size +=
				sizeof(struct json_array_element_s) * elements;
...
json_get_number_size(struct json_parse_state_s *state)
{
	...
	if (offset < size) {
		...
		case ']':
			...
			break;
		case '=':
			if (json_parse_flags_allow_equals_in_object &
			    flags_bitset) {
				break;
			}
...
json_get_value_size(struct json_parse_state_s *state, int is_global_object)
{
	...
	if (is_global_object) {
		...
		case '+':
			if (json_parse_flags_allow_leading_plus_sign &
			    flags_bitset) {
				return json_get_number_size(state);
			} else {
				/* invalid value! */
		...
		default:
			if ((offset + 4) <= size && 't' == src[offset + 0] &&
				...
				state->offset += 4;
				return 0;
			} else if ((json_parse_flags_allow_inf_and_nan &
				    flags_bitset) &&
				   (offset + 3) <= size &&
				   'N' == src[offset + 0] &&
				   'a' == src[offset + 1] &&
...
static void json_parse_string(struct json_parse_state_s *state,
			      struct json_string_s *string);
static void
json_parse_string(struct json_parse_state_s *state,
		  struct json_string_s *string)
{
	size_t offset = state->offset;
	...
	while (quote_to_use != src[offset]) {
		if ('\\' == src[offset]) {
			...
			case 'u': {
				codepoint = 0;
				if (!json_hexadecimal_value(&src[offset], 4,
							    &codepoint)) {
...
static void
json_parse_object(struct json_parse_state_s *state, int is_global_object,
		  struct json_object_s *object)
{
	const size_t flags_bitset = state->flags_bitset;
	const size_t size = state->size;
...
static void
json_parse_number(struct json_parse_state_s *state,
		  struct json_number_s *number)
{
	const size_t flags_bitset = state->flags_bitset;
	size_t offset = state->offset;
...
static void json_parse_value(struct json_parse_state_s *state,
			     int is_global_object, struct json_value_s *value);
static void
json_parse_value(struct json_parse_state_s *state, int is_global_object,
		 struct json_value_s *value)
{
	const size_t flags_bitset = state->flags_bitset;
	const char *const src = state->src;
	...
	if (is_global_object) {
		...
		case '\'':
			...
			state->dom += sizeof(struct json_string_s);
			json_parse_string(
				state, (struct json_string_s *)value->payload);
			break;
		case '{':
			value->type = json_type_object;
			value->payload = state->dom;
	      ...
	      void *user_data, struct json_parse_result_s *result)
{
	...
	input_error = json_get_value_size(
		&state, (int)(json_parse_flags_allow_global_object &
			      state.flags_bitset));
	...
	json_parse_value(&state,
			 (int)(json_parse_flags_allow_global_object &
			       state.flags_bitset),
			 value);
...
static struct json_extract_result_s
...
static void json_extract_copy_value(struct json_extract_state_s *const state,
				    const struct json_value_s *const value);
static void
json_extract_copy_value(struct json_extract_state_s *const state,
			const struct json_value_s *const value)
{
	struct json_string_s *string;
	struct json_number_s *number;
	...
	if (json_type_string == value->type) {
		memcpy(state->dom, value->payload,
		       sizeof(struct json_string_s));
		string = (struct json_string_s *)state->dom;
		state->dom += sizeof(struct json_string_s);
		...
		for (i = 0; i < object->length; i++) {
			...
			memcpy(state->dom, element,
			       sizeof(struct json_object_element_s));
			element = (struct json_object_element_s *)state->dom;
			state->dom += sizeof(struct json_object_element_s);

			string = element->name;
			memcpy(state->dom, string,
			       sizeof(struct json_string_s));
			string = (struct json_string_s *)state->dom;
			state->dom += sizeof(struct json_string_s);
			element->name = string;

			memcpy(state->data, string->string,
		...
		for (i = 0; i < array->length; i++) {
			...
			memcpy(state->dom, element,
			       sizeof(struct json_array_element_s));
			element = (struct json_array_element_s *)state->dom;
			state->dom += sizeof(struct json_array_element_s);
...
json_write_get_number_size(const struct json_number_s *number, size_t *size)
{
	...
	if (number->number_size >= 2) {
       ...
       * output. */
			parsed_number =
				json_strtoumax(number->number, json_null, 0);
...
json_write_number(const struct json_number_s *number, char *data)
{
	...
	if ((i < number->number_size) && ('I' == number->number[i])) {
		...
		if ('\0' == *inf++) {
			...
			/* Inf becomes 1.7976931348623158e308 because JSON can't support it. */
			for (dbl_max = "1.7976931348623158e308";
			     '\0' != *dbl_max; dbl_max++) {
				*data++ = *dbl_max;
			}
...
json_write_minified_value(const struct json_value_s *value, char *data)
{
	...
	default:
		...
		return json_null;
	case json_type_number:
		return json_write_number((struct json_number_s *)value->payload,
					 data);
	case json_type_string:
		return json_write_string((struct json_string_s *)value->payload,
					 data);
	case json_type_array:
...
static int json_write_pretty_get_array_size(const struct json_array_s *array,
					    ...
					    size_t newline_size, size_t *size);
static int
json_write_pretty_get_array_size(const struct json_array_s *array, size_t depth,
				 size_t indent_size, size_t newline_size,
				 size_t *size)
{
	struct json_array_element_s *element;
		...
		for (element = array->start; json_null != element;
			...
			*size += (depth + 1) * indent_size;
			...
			if (json_write_pretty_get_value_size(
				    element->value, depth + 1, indent_size,
				    newline_size, size)) {
				/* value was malformed! */
				return 1;
			}
	...
	if (0 < object->length) {
		...
		*size += object->length - 1; /* ','s seperate each element. */
		...
		for (element = object->start; json_null != element;
			...
			*size +=
				3; /* seperate each name/value pair with " : ". */
...
static int json_write_pretty_get_value_size(const struct json_value_s *value,
					    ...
					    size_t newline_size, size_t *size);
static int
json_write_pretty_get_value_size(const struct json_value_s *value, size_t depth,
				 size_t indent_size, size_t newline_size,
				 size_t *size)
{
	switch (value->type) {
	...
	case json_type_object:
		return json_write_pretty_get_object_size(
			(struct json_object_s *)value->payload, depth,
			indent_size, newline_size, size);
	case json_type_true:
		*size += 4; /* the string "true". */
		return 0;
...
static char *json_write_pretty_array(const struct json_array_s *array,
				     ...
				     const char *newline, char *data);
static char *
json_write_pretty_array(const struct json_array_s *array, size_t depth,
	...
	*data++ = '['; /* open the array. */
	...
	if (0 < array->length) {
		...
		for (element = array->start; json_null != element;
			...
			data = json_write_pretty_value(element->value,
						       depth + 1, indent,
						       newline, data);
...
static char *json_write_pretty_object(const struct json_object_s *object,
				      ...
				      const char *newline, char *data);
static char *
json_write_pretty_object(const struct json_object_s *object, size_t depth,
			 const char *indent, const char *newline, char *data)
{
	size_t k, m;
	struct json_object_element_s *element;
...
static char *json_write_pretty_value(const struct json_value_s *value,
				     ...
				     const char *newline, char *data);
static char *
json_write_pretty_value(const struct json_value_s *value, size_t depth,
			const char *indent, const char *newline, char *data)
{
	switch (value->type) {
	default:
	...
	case json_type_array:
		return json_write_pretty_array(
			(struct json_array_s *)value->payload, depth, indent,
			newline, data);
	case json_type_object:
		return json_write_pretty_object(
			(struct json_object_s *)value->payload, depth, indent,
			newline, data);
	case json_type_true:
		data[0] = 't';
		data[1] = 'r';
...
static void *
json_write_pretty(const struct json_value_s *value, const char *indent,
		  const char *newline, size_t *out_size)
{
	...
	if (json_write_pretty_get_value_size(value, 0, indent_size,
					     newline_size, &size)) {
		/* value was malformed! */
		return json_null;
	}
